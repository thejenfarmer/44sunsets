...
// 15a — scheduled session with the call companion: pre → during (presence glow) → Stack landing.
    ...
    return (
      <Shell bg="#FAF3E7">
        ...
        <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', padding: '48px 20px 20px', minHeight: 0 }}>
          ...
          <p style={{ margin: 0, fontSize: 13.5, color: 'rgba(34,26,18,.55)' }}>{who} starts at {s.call?.at ?? '2:00'}.</p>
          ...
          <DarkPill onClick={() => setStep('during')}>Sit down with {who} →</DarkPill>
        ...
        <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', padding: '48px 20px 20px' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 22 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13.5, color: 'rgba(34,26,18,.6)' }}>
              <span className="glow" style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#F4A69B,#F6C95C)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: 12 }}>{who[0]}</span>
              {who}’s still at their desk.
            </div>
          </div>
      ...
      <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', padding: '48px 20px 20px' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, textAlign: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            <span className="glow" style={{ width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg,#F4A69B,#F6C95C)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: 24, color: '#221A12' }}>{who[0]}</span>
            <div style={{ fontSize: 14, color: 'rgba(34,26,18,.7)' }}>{who}’s here</div>
